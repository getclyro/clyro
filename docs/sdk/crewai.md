# Clyro SDK — CrewAI Usage Guide

Add observability and execution controls to CrewAI agents with a single line. Clyro captures LLM calls with token counts, tool usage, and task lifecycle events — without changing how your crew works.
...
Requires Python ≥ 3.11. Requires CrewAI ≥ 0.30.0.
...
task = Task(description="Research the latest AI regulations", agent=researcher)
crew = Crew(agents=[researcher], tasks=[task])

result = crew.kickoff(inputs={"topic": "AI regulations"})
```
...
wrapped = clyro.wrap(crew)  # one line added

result = wrapped.kickoff(inputs={"topic": "AI regulations"})  # same call
```

Clyro injects into CrewAI's event bus. It captures LLM calls with token counts, tool usage, and task lifecycle events.
...
| `task_start` / `task_end` | CrewAI task lifecycle |
...
export CLYRO_AGENT_NAME="my-crew"
...
wrapped = clyro.wrap(crew)
result = wrapped.kickoff(inputs={"topic": "AI regulations"})
```
...
wrapped = clyro.wrap(crew, config=config)
```
...
config = ClyroConfig(
    ...
    agent_name="my-crew",             # human-readable name; auto-registers on first run
...
# Inside your tool or agent logic:
