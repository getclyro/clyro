...
def _config_with_rules(rules: list[dict]) -> WrapperConfig:
    return WrapperConfig.model_validate({"default_action": "allow","global": {"policies": rules}})


def _config_with_tool_rules(tool_name: str, rules: list[dict]) -> WrapperConfig:
    return WrapperConfig.model_validate({"default_action": "allow","tools": {tool_name: {"policies": rules}}})
...
class TestDualPathEvaluation:
    ...
    def test_all_rules_evaluated_even_after_violation(self) -> None:
        ...
        cfg = _config_with_rules([
            {"action": "block", "parameter": "amount", "operator": "max_value", "value": 100, "name": "max_amount"},
            {"action": "block", "parameter": "currency", "operator": "equals", "value": "USD", "name": "currency_check"},
        ])
        ...
        assert rule_results[0]["outcome"] == "matched"
        # Second rule: currency=USD matches equals 'USD' → also matched (block).
        assert rule_results[1]["outcome"] == "matched"

    def test_no_violation_returns_all_no_match(self) -> None:
        """When no rules match, all rule_results should be 'no_match'."""
        cfg = _config_with_rules([
            {"action": "block", "parameter": "amount", "operator": "max_value", "value": 1000},
            {"action": "block", "parameter": "currency", "operator": "equals", "value": "EUR"},
        ])
        pe = PolicyEvaluator(cfg)
        ...
        assert details == {}
        assert len(rule_results) == 2
        assert all(r["outcome"] == "no_match" for r in rule_results)
...
class TestRuleResultStructure:
    ...
    def test_matched_rule_result_fields(self) -> None:
        """Matched rule result has all required fields."""
        policy_id = str(uuid4())
        cfg = _config_with_rules([
    ...
    def test_no_match_rule_result_fields(self) -> None:
        """No-match rule result has null message."""
        cfg = _config_with_rules([
            {"action": "block", "parameter": "amount", "operator": "max_value", "value": 1000, "name": "Limit"},
        ])
        pe = PolicyEvaluator(cfg)
    ...
    def test_per_tool_and_global_rules_combined(self) -> None:
        ...
        cfg = WrapperConfig.model_validate({"default_action": "allow",
            ...
            "tools": {"mytool": {"policies": [
                {"action": "block", "parameter": "x", "operator": "max_value", "value": 5, "name": "tool_limit"},
            ]}},
        })
        pe = PolicyEvaluator(cfg)
        ...
        assert rule_results[0]["outcome"] == "matched"
        # x=10 not > 999 → global rule's condition does not match.
        assert rule_results[1]["outcome"] == "no_match"
